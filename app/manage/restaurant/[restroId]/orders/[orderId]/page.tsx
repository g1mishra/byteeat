...
async function OrderTable({params}: {params: {restroId: string, orderId: string}}) {
    const order = await getOrderById(params.orderId);

    const orderTotal = order?.orderItems?.reduce((acc, orderItem) => acc + orderItem.price * orderItem.quantity, 0);
            ...
            <Table>
                ...
                <TableBody>
                    {order?.orderItems?.map((orderItem) => (
                         ...
                         <TableCell className="text-right"> &#x20b9; {orderItem.price}</TableCell>
                         </TableRow>
                    ))}
                <TableRow>
