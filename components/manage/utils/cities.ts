...
const state2city: any = {}

for (const idx in citieslist) {
  const city = citieslist[idx]
  if (city.state in state2city) {
    if (!state2city[city.state].includes(city.name)) {
      state2city[city.state].push(city.name)
    }
  } else {
    state2city[city.state] = [city.name]
  }
}
